...
const playBtn = document.querySelector(".how-it-works-video");
const playImg = document.querySelector(".how-it-works-blur");
...
playBtn.addEventListener("click", (e) => {
  playBtn.style.padding = "0px";
  player.style.display = "block";
  playImg.style.display = "none";
  player.play();
});

const challengeQuests = document.querySelectorAll(".challenges__question");

challengeQuests.forEach((elem) => {
  elem.addEventListener("click", (e) => {
    let target = e.target.closest(".challenges__question");
    if (target.classList.contains("challenges__question-active")) return;
    challengeQuests.forEach((el) => {
      el.classList.remove("challenges__question-active");
    });

    target.classList.add("challenges__question-active");
  });
});

const screen = document.querySelector(".how-it-works-video");
